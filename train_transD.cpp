...
int norm(float *con, int dimension) {
    return 0;
	float x = 0;
...
int norm(float *con, float *conTrans, float *relTrans) {
    return 0;
...
float calc_sum(int e1, int e2, int rel, float *tmp1, float *tmp2, float &ee1, float &ee2) {
	...
	for (int ii = 0; ii < dimensionR; ii++) {
        ...
        if (L1_flag)
		    ...
		    sum += sqr(tmp1[ii] + relationVec[lastr + ii] - tmp2[ii]);
	}
	return sum;
}
